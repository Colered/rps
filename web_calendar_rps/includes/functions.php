...
function html_for_event_day_at_a_glance ( $event, $date ) {
  ...
  . ( $can_access != 0 && $time_only != 'Y'? 'href="view_entry.php?id=' . $id . '&amp;date=' . ( $getClone ? $getClone : $date ) . ( strlen ( $GLOBALS['user'] ) > 0
      ? '&amp;user=' . $GLOBALS['user']
...
function print_entry ( $event, $date ) {
  ...
  if ( empty ( $viewEventStr ) ) {
    $viewEventStr = translate ( '' );
    $viewTaskStr = translate ( 'View this taskss' );
  }
...
function read_events_student_sub_next_sem ( $user, $startdate, $enddate, $cat_id = '',$subject_id='') {
  ...
  $start_date = gmdate ( 'Ymd', $startdate );
  $end_date = gmdate ( 'Ymd', $enddate );
  return query_events_student_sub_next_sem ( $user, false, ' AND ( ( we.cal_date >= ' . $start_date
     . ' AND we.cal_date <= ' . $end_date
     . ' AND we.cal_time = -1 ) OR ( we.cal_date > ' . $start_date
     . ' AND we.cal_date < ' . $end_date . ' ) OR ( we.cal_date = ' . $start_date
     . ' AND we.cal_time >= ' . gmdate ( 'His', $startdate )
  ...
  if(count($row)>0){
    ...
    for ( $ii = 0, $cnt = count ( $rows ); $ii < $cnt; $ii++ ) {
	  ...
	  if ( $login == '__public__' && ! empty ( $OVERRIDE_PUBLIC ) && $OVERRIDE_PUBLIC == 'Y' ) {
	   ...
	   $row[1] = (isset($row[1])) ? ($row[1]) : '';//date
	   $row[2] = (isset($row[2])) ? ($row[2]) : '';//timeslot
	   $eventstartdate = '';
		if($row[1]!=''){
		 $splitTimeslot = (isset($row[2])) ? ($row[2]) : '';
		 $start_timeslot=$splitTimeslot;
		 ...
		 //$entry_hour_st.' #'. $entry_minute_am_pm_st.'#'.$ts_array1['1'];
		 $entry_time_et = (isset($ts_array1['1']) && $ts_array1['1']!='')? (date("H:i",strtotime(trim($ts_array1['1'])))) : ''; 
		 ...
		 $entry_minute_am_pm_et = (isset($entry_et_array['1'])) ? ($entry_et_array['1']) : '';
		 $entry_minute_am_pm__et_arr=(isset($entry_minute_am_pm_et)) ? (explode(" ",$entry_minute_am_pm_et)) : '';
		 $entry_minute_et=(isset($entry_minute_am_pm__et_arr['0'])) ? ($entry_minute_am_pm__et_arr['0']) : '';
		 $duration = abs($et - $st) / 60;
		 $date_array = (isset($row[1])) ? (explode("-",date('Y-m-d',strtotime($row[1])))) : '';
		 ...
		 $evt_name='';
		 $teacher=(isset($row[3]) && $row[3]!='')?$row[3]:'';
		 $teacher_type=(isset($row[4]) && $row[4]!='')?$row[4]:'';
		 $program_year_name=(isset($row[5]) && $row[5]!='')?$row[5]:'';
		 $subject=(isset($row[11]) && $row[11]!='')?$row[11]:'';
		 $session=(isset($row[9]) && $row[9]!='')?$row[9]:'';
		 $room=(isset($row[14]) && $row[9]!='')?$row[14]:'';
		 $rule=(isset($row[15]) && $row[15]!='')?'<strong>Rule:-</strong>'.$row[15]:'';
		 $evt_descr ='<strong>Program:-</strong>'.$program_year_name.'<br>'.'<strong>Subject:-</strong>'.$subject.'<br>'.'<strong>Session:-</strong>'.$session.'<br>'.$rule.'<br>'.'<strong>Teacher:-</strong>'.$teacher.'<br>'.'<strong>Teacher Type:-</strong>'.$teacher_type.'<br>'.'<strong>Room:-</strong>'.$room.'<br>'.'<strong>Date:-</strong>'.date('d-m-Y',strtotime($row[1]));
		}
	}
	if ( $want_repeated && ! empty ( $row[20] ) ) {// row[20] = cal_type
...
function print_subject_menu ( $form, $date = '', $cat_id = '',$subject_filter_id ) {
  ...
  $obj_ras=new RAS();
  $subjectData=$obj_ras->getProgramSub();
  if ( empty ( $CATEGORIES_ENABLED  ) || $CATEGORIES_ENABLED == 'N' )
    return false;
  ...
  if ( is_array ( $subjectData ) ) {
    ...
    foreach ( $subjectData as $K => $V ) {
      if ( ( ! empty ( $user ) && strlen ( $user ) ? $user : $login )) {
       $ret .= '
			<option value="' . $subjectData[$K]['ras_subject_id'] . '"';
			if ( $subject_filter_id == $subjectData[$K]['ras_subject_id'] ) {
			  $printerStr .= '<span id="cat">' . $catStr . ': ' . $subjectData[$K]['subject_name'] . '</span>';
			  $ret .= ' selected="selected"';
			 }
